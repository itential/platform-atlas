...
If your deployment includes Gateway 4, collect the following from the Gateway
server:
...
If your deployment uses Gateway 5 (containerized), provide one of the following:
...
Gateway 5 sources—the server `gateway.conf` (INI) and SSH `printenv`—are
