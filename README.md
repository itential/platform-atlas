...
- **Unified Report**—`session run report` generates a single standalone `report.html` folding Compliance, Operational, and Architecture into one tabbed file, and opens it automatically in the browser
- **RBAC Analysis (early feature)**—opt-in capture of Platform 6's authorization model, surfaced as a dedicated RBAC tab in the report: an identity privilege heatmap, a group privilege heatmap, and an identity × group membership matrix. Toggled from the **Additional Validation Modules** checklist (see below), not a standalone setting
- **Additional Validation Modules**—every Extended Validation check (adapter versions, Redis ACL, Mongo WiredTiger cache, RBAC, log analysis, and the rest) can be turned off individually from `config edit` → Advanced, without touching rulesets. A disabled check shows as "Module Deactivated" in the report rather than looking like a failure. All modules default to enabled except RBAC, which stays opt-in
...
Platform Atlas can use HashiCorp Vault as a read-only credential backend instead of the OS keyring. In this mode, Atlas reads credentials from a KV v2 secrets engine at runtime but never writes to Vault—secrets are managed externally through the Vault UI, CLI, or API calls outside of Atlas.
...
`config doctor` is a one-shot diagnostic that verifies the global config, active environment, credential backend, Platform/Gateway URL reachability, active ruleset, and SSH key path in a single pass. It also reports the total on-disk size of `~/.atlas` under a Runtime line, so you can see at a glance how much space sessions, captures, and reports are using. Use it after `config init`, after editing an environment, or any time a capture fails for an unclear reason—it surfaces every issue at once rather than letting them appear one-by-one across multiple capture runs. Exits non-zero on warnings or errors so it composes cleanly with shell scripts and CI.
...
These fields are also editable after setup via `platform-atlas env edit` → Deployment Topology → **Edit Kubernetes settings** (namespace, context, values.yaml paths, kubectl binary), or the WebUI **Environments** form—no need to re-run the full topology wizard for a Kubernetes environment.

### Additional namespaces

For the rare case of a second Platform or Gateway5 deployment living in its own namespace or cluster, an advanced, opt-in **Additional namespaces** option (in `env create`/`env edit` → Deployment Topology → Edit Kubernetes settings, the `tier upgrade` walkthrough, or the browser setup form) captures and validates each extra namespace independently, with its own pass/fail results shown in the report.
...
When an environment includes a Gateway 5, Atlas reads its `GATEWAY_*` settings from one of four sources, chosen during `env create` / `env edit`:
...
- **Helm values file**—parses `env` / `extraEnv` and the Gateway 5 chart's `serverSettings` / `applicationSettings` / `runnerSettings`.
- **Server `gateway.conf` over SSH**—reads the Gateway 5 server config file (INI); only a `server`-mode file is accepted.
...
### MongoDB / Redis via a jumphost tunnel (Extended tier)

For Extended-tier deployments where Atlas can't reach MongoDB or Redis directly—only through a bastion host you control—an advanced, opt-in setting reuses the same ControlMaster mechanism to tunnel those two protocol connections through a jumphost. You open the jumphost's own SSH session yourself (Atlas never holds jumphost credentials); Atlas then pushes a local port-forward onto that socket for whichever of Mongo/Redis you select, with a live connectivity test before anything is saved.

Configure it in `env create`/`env edit` (look for **Jumphost Tunnel** under Deployment Topology), the `tier upgrade` walkthrough, or either browser form. `session run capture` checks the jumphost's socket alongside your other ControlMaster nodes before capture starts, and offers to open it automatically if it isn't ready—same status table, same `--resume` behavior as node-level ControlMaster above.
...
`session run report` generates a single standalone `report.html` with three pages, switched from a persistent top bar:
...
| `session run report` | Generate `report.html` (compliance, operational, architecture as pages in one file) |
...
| Command | Description |
|---|---|
| `env list` | List all environments with description, platform URI, mode, backend, and active status (`⚠ incomplete` shown for partial setups) |
...
Please see the separate guide entitled `SSH_SETUP_GUIDE` for full details on how to set up SSH access for all servers.
...
### TLS for MongoDB / Redis connections

`env create`/`env edit` offer an independent **Enable TLS** toggle for each of MongoDB and Redis, so a deployment can require TLS on one and not the other. When enabled, Atlas transparently upgrades the stored connection URI at connect time—appending `tls=true&tlsInsecure=true` for MongoDB, or switching to the `rediss://` scheme with certificate verification disabled for Redis. This encrypts the connection in transit but does not verify the server's certificate, since these audits overwhelmingly target self-signed certificates; custom CA files and mutual TLS aren't supported yet.
...
Existing sessions and environments continue to work without changes. Sessions created before 1.5 won't have bound metadata, but you can backfill it from capture data with `platform-atlas session repair`.
